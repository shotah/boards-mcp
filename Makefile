# boards-mcp Makefile — Go training wheels for the Python-brained
#
# Run `make` or `make help` to see everything.
# Do not git init here; the human creates the GitHub repo.
...
LDFLAGS := -X github.com/shotah/boards-mcp/server.ServerVersion=$(VERSION)
...
help: ## Show this help
	...
	@echo "  run                    go run  (make run ARGS=\"roster list\")"
	@echo ""
...
cli: ## Build the MCP binary into ./bin/boards-mcp
	mkdir -p bin
	CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" -o bin/$(BINARY)$(EXE) .

install: ## Install binary into $$GOPATH/bin (or $$GOBIN) as boards-mcp
	CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" -o "$(GOBIN_DIR)/$(BINARY)$(EXE)" .

run: ## Build & run — e.g. make run ARGS="roster list"
